...
describe("Treeable repository", () => {
  ...
  it("imports installed executable skills by skill name and updates repeated imports", () => {
    const repo = createTreeableRepository(testDbPath());
    ...
    expect(repo.listSkills().filter((skill) => skill.id === "xiaohongshu-skills")).toHaveLength(1);
  });

  it("discovers installed skills directly from the skill folder", () => {
    ...
    const discovered = repo.listSkills().find((skill) => skill.id === "local-travel");
    ...
    expect(repo.resolveSkillsByIds(["local-travel"]).map((skill) => skill.id)).toEqual(["local-travel"]);
  });

  it("cleans old imported skill runtime paths from stored prompts", () => {
    const dbPath = testDbPath();
    ...
    const skill = reopened.listSkills().find((item) => item.id === "legacy-installed");

    expect(skill?.prompt).toBe(["# Root Skill", "# Legacy", "", "保留真实指令。"].join("\n"));
  });

  it("can enable a skill that was added directly to the skill folder", () => {
  ...
  it("persists completed tool query memory on generated session outputs", () => {
    const repo = createTreeableRepository(testDbPath());
    const root = repo.saveRootMemory({
      seed: "写一篇青岛旅行攻略",
      domains: ["旅行"],
    ...
    expect((repo.getSessionState(draftState.session.id) as any).toolMemory).toContain("青岛三天两晚攻略");
  });
